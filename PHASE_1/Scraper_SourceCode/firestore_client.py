...
class FireStore_Client:
    ...
    def write_disease_auto_id(self, name: str, symptoms: str) -> str:
        """
    ...
    def write_disease(self, disease_id: str, name: str, symptoms: str):
        """
        ...
        disease_ref.set({
            ...
            u'name':                 name,
            u'symptoms':             parsed_symptoms,
        })

    def write_article_auto_id(self, url: str, headline: str, main_text: str, date_of_publication: str) -> str:
        """
        Same as 'write_article' except you don't have the article_id.
        Return the article_id hash.
        """
        article_id = hashlib.md5((url + headline + main_text + date_of_publication).encode("utf-8")).hexdigest()
        self.write_article(article_id, url, headline, main_text, date_of_publication)
        return article_id

    def write_article(self, article_id: str, url: str, headline: str, main_text: str, date_of_publication: str):
        """
        Writes an article to the DB.
        """
        disease_ref = self.db.collection(u'articles').document()

        # Parse date_of_publication from str (format YYYY-MM-DD) to datetime.
        parsed_datetime = datetime.strptime(date_of_publication, "%Y-%m-%d")
        ...
        disease_ref.set({
            ...
            u'date_of_publication': parsed_datetime
        })

    def write_report_auto_id(self, article_id: str, disease_id: str, location: str, event_date: str, reported_cases: int = None, hospitalisations: int = None, deaths: int = None) -> str:
        """
        Same as 'write_report' except you don't have the report_id.

        Returns the report_id hash
        """
        strings = [article_id, disease_id, location, event_date, str(reported_cases), str(hospitalisations), str(deaths)]
    ...
    def write_report(self, report_id: str, article_id: str, disease_id: str, location: str, event_date: str, reported_cases: int = None, hospitalisations: int = None, deaths: int = None):
        """
        Writes a report to the DB.
        """
        disease_ref = self.db.collection(u'reports').document()

        # Parse event_date from str (format YYYY-MM-DD) to datetime.
        ...
        disease_ref.set({
            ...
            u'article_id': article_id,
            u'disease_id': disease_id,
            u'event_date': parsed_event_date,
            u'location':   location,
            u'statistics': {
